...
1. **Auto-detección y Selección de Candidatos**:
   - **Modo completo** (default): enriquecer **todos** los productos cargados. `--limite` = `COUNT(*)` de `{schema}.productos`. No recortar a 100.
   - **Modo demo** (`manifest.modo = demo`): enriquecer **todos** los productos cargados en Fase 1 (80–100). No preguntar un cupo menor ni omitir esta fase.
   - Brindar la estimación de tiempo correspondiente en base a **~3 segundos por producto**.
   ...
   - Correr la recopilación de candidatos con el límite = todos los cargados:
     ```bash
     python scripts/fase-01-catalogo/buscar_candidatos.py --esquema {schema} --limite {limite}
     ```
2. **Ejecutar Enriquecimiento (Dry Run)**:
   - Ejecutar el script `scripts/fase-01-catalogo/enriquecer_catalogo.py` para generar descripciones y alias de calidad:
     ```bash
     python scripts/fase-01-catalogo/enriquecer_catalogo.py --esquema {schema} --csv-entrada implementacion/{schema}/inputs/candidatos_a_enriquecer.csv --csv-salida implementacion/{schema}/outputs/vista_previa_enriquecimiento.csv
     ```
...
4. **Persistir Cambios y Re-vectorizar**:
   - Aplicar los cambios a la base de datos de Supabase y encolar la re-vectorización en el backend:
     ```bash
     python scripts/fase-01-catalogo/enriquecer_catalogo.py --esquema {schema} --aplicar --csv-entrada implementacion/{schema}/outputs/vista_previa_enriquecimiento.csv
     ```

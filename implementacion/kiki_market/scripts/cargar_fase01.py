#!/usr/bin/env python3
"""Carga Fase 1 catálogo en schema kiki_market (lista única, precios -40%).

schema_name = kiki_market (confirmado por el implementador: confirmar carga).
"""
...
async def cargar() -> int:
    ...
    print(f"[*] tenant destino: {SCHEMA} (Kiki Market, lista única)")
    ...
    if len(products) < 1:
        print("[FAIL] phase-01-productos.csv vacío", file=sys.stderr)
        return 1
    ...
    try:
        ...
        await conn.execute(
            ...
            VALUES
              (1, 'Lista única', 'Precio Tienda Nube menos 40%', true, true, false, now(), now())
        ...
        prices_data = []
        with (OUT / "phase-01-lista-precios-1.csv").open(encoding="utf-8") as f:
        ...
        print(f"[*] Insertando lista 1: {len(prices_data)} precios en {SCHEMA}...")
        ...
        if verify["productos"] != len(products) or verify["precios"] != len(products):
            print("[FAIL] Conteos no coinciden con el CSV", file=sys.stderr)
            return 1
        if verify["listas"] != N_LISTAS:
            print("[FAIL] La lista única debe estar activa y pública", file=sys.stderr)
            return 1
    finally:
        await conn.close()
    ...
    try:
        resp = requests.post(vec_url, json=product_codes, timeout=120)

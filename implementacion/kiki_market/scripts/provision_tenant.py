#!/usr/bin/env python3
"""Alta de tenant kiki_market: clone gonzales + fila en public.distribuidoras + owner.

schema_name = kiki_market (confirmado: Kiki Market, Villa Carlos Paz).
Tenant nuevo / vacío. Lista única. Catálogo real (is_mock=false).
"""
...
SCHEMA = "kiki_market"  # confirmado: Kiki Market
SOURCE = "gonzales"
ADMIN_EMAIL = "admin@kikimarket.com"  # dominio sin underscore (DNS/Supabase Auth)
...
def create_auth_user() -> str:
    ...
    resp = requests.post(
        ...
        json={
            ...
            "email_confirm": True,
            "user_metadata": {"nombre": "Admin Kiki Market"},
        },
        timeout=30,
    )
...
async def complete_owner(conn: asyncpg.Connection, tenant_id: str, created_at) -> None:
    existing_profile = await conn.fetchval(
        "SELECT 1 FROM public.profiles WHERE distribuidora_id = $1 AND role = 'owner' LIMIT 1",
        tenant_id,
    )
    if existing_profile:
        print(f"[*] owner ya existe para {SCHEMA}")
        return
    ...
    project = await conn.fetchrow(
        ...
        VALUES ($1, COALESCE($2, now()), 'agentic_implementation')
...
async def main() -> int:
    ...
    print(f"[*] tenant destino: {SCHEMA} (nuevo / vacío, Kiki Market)")
    ...
    try:
        ...
        existing = await conn.fetchrow(
            "SELECT id, schema_name, nombre, created_at FROM public.distribuidoras WHERE schema_name = $1",
            SCHEMA,
        )
        if existing:
            print(f"[*] Ya existe public.distribuidoras schema_name={SCHEMA} id={existing['id']}")
            await complete_owner(conn, str(existing["id"]), existing["created_at"])
    ...
    try:
        ...
        tenant_id = uuid4()
        metadata = {
            "ciudad_base": "Villa Carlos Paz, Córdoba, Argentina",
            "hq": {
                "label": "Av. Arturo Umberto Illia 647, Villa Carlos Paz, Córdoba",
                "latitude": -31.4240,
                "longitude": -64.4978,
            },
            "marca_comercial": "Kiki Market",
            "sitio_web": "https://kikimarket.com.ar",
            "origen_implementacion": "fase-00-preflight",
            "modo": "completo",
            "lista_unica": True,
            "precio_factor_tiendanube": 0.60,
        }

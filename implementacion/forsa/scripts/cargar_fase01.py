#!/usr/bin/env python3
"""Carga Fase 1 catálogo demo en schema forsa desde CSVs."""
...
SCHEMA = "forsa"  # confirmado dos veces: implementador + manifest
...
async def cargar() -> int:
    ...
    try:
        ...
        print("[*] Insertando 4 listas de precios (activa+es_publica) en forsa...")
        await conn.execute(
            f"""
        ...
        print(f"[*] Insertando {len(products_data)} productos en forsa...")
        ...
        if aliases_data:
            print(f"[*] Insertando {len(aliases_data)} aliases en forsa...")
        ...
        for list_id in range(1, N_LISTAS + 1):
            ...
            print(f"[*] Insertando lista {list_id} en forsa: {len(prices_data)} precios...")
    ...
    print("OK carga catálogo forsa completada")
    return 0

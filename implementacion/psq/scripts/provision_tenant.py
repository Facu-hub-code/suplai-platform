#!/usr/bin/env python3
"""Alta de tenant psq: clone gonzales + fila en public.distribuidoras + owner.

schema_name = psq (confirmado por el implementador). Tenant nuevo / vacío (demo).
...
def create_auth_user() -> str:
    ...
    resp = requests.post(
        ...
        json={
            ...
            "email_confirm": True,
            "user_metadata": {"nombre": "Admin PSQ"},
        },
        timeout=30,
    )
...
async def main() -> int:
    ...
    try:
        ...
        row = await conn.fetchrow(
            ...
            str(tenant_id),
            "PSQ",
            "PSQ Artículos de Limpieza y Descartables",
            SCHEMA,

#!/usr/bin/env python3
"""Carga Fase 1 catálogo en schema cipres desde CSVs (confirmación implementador)."""
...
SCHEMA = "cipres"  # confirmado dos veces: implementador + manifest
...
async def cargar() -> int:
    ...
    try:
        ...
        print("[*] Insertando 3 listas de precios (activa+es_publica) en cipres...")
        await conn.execute(
            f"""
            ...
            VALUES
              (1, 'Mayorista', 'Lista mayorista / revendedores (precioventa2)', true, true, true, now(), now()),
              (2, 'Intermedia', 'Lista intermedia (precioventa3)', true, true, true, now(), now()),
              (3, 'Público', 'Lista público / mostrador (precioventa5)', true, true, true, now(), now())
        ...
        print(f"[*] Insertando {len(products_data)} productos en cipres...")
        ...
        if aliases_data:
            print(f"[*] Insertando {len(aliases_data)} aliases en cipres...")
        ...
        for list_id in range(1, N_LISTAS + 1):
            ...
            print(f"[*] Insertando lista {list_id} en cipres: {len(prices_data)} precios...")
        ...
        if verify["precios"] != expected_prices:
            print(
                f"[FAIL] Conteo precios {verify['precios']} != esperado {expected_prices}",
                file=sys.stderr,
            )
            return 1
        if verify["listas"] != N_LISTAS:
            print(
                f"[FAIL] Conteo listas {verify['listas']} != {N_LISTAS}",
                file=sys.stderr,
            )
            return 1
    ...
    print("OK carga catálogo cipres completada")
    return 0

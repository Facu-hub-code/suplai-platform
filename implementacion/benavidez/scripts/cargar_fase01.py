#!/usr/bin/env python3
"""Carga Fase 1 catálogo en schema benavidez desde CSVs (confirmación implementador)."""
...
async def cargar() -> int:
    ...
    print(f"[*] destino de carga: {SCHEMA}")
    ...
    try:
        ...
        resume_prices_only = (
            counts["productos"] == len(products)
            and counts["precios"] == 0
            and counts["listas"] >= N_LISTAS
        )
        if counts["productos"] > 0 and not resume_prices_only:
            print(
                f"[FAIL] {SCHEMA} ya tiene datos (productos={counts['productos']}, precios={counts['precios']}). Aborto.",
                file=sys.stderr,
            )
            return 1
        if resume_prices_only:
            print(
                f"[*] Reanudando: productos={counts['productos']} aliases={counts['aliases']} "
                f"listas={counts['listas']} precios=0. Solo inserto precios."
            )

        if not resume_prices_only:
            print(f"[*] Insertando {N_LISTAS} listas de precios (activa+es_publica) en {SCHEMA}...")
            await conn.execute(
                f"""
        ...
        if not resume_prices_only:
            ...
            if aliases_data:
                ...
                for i in range(0, len(aliases_data), BATCH):
                    ...
                    print(f"  … aliases {min(i + BATCH, len(aliases_data))}/{len(aliases_data)}")
        ...
        for s in sample:
            print(
                f"  sample {s['product_code']} | {s['nombre'][:50]} | L{s['lista_precios_id']}={s['precio_unidad']}"
            )
    ...
    try:
        ...
        for i in range(0, len(product_codes), 200):
            ...
            resp = requests.post(vec_url, json=chunk, timeout=90)
            if resp.status_code == 200:
                scheduled += len(chunk)
                print(f"  … vectorize OK chunk {i // 200 + 1} ({resp.text[:160]})")
            else:

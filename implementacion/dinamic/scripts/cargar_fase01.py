#!/usr/bin/env python3
"""Carga Fase 1 catálogo en schema dinamic desde CSVs (demo 95 SKUs).

schema_name = dinamic (confirmado: implementar + confirmar carga).
"""
...
async def cargar() -> int:
    ...
    if not (80 <= len(products) <= 100):
        print(f"[FAIL] Demo espera 80–100 SKUs, hay {len(products)}", file=sys.stderr)
        return 1
    ...
    try:
        ...
        print(f"[*] INSERT {SCHEMA}.listas_precios (4 listas públicas)")
        await conn.execute(
            f"""
            ...
            VALUES
              (1, 'Lista 1', 'Lista Base (Público) — lista blanca agosto 2026', true, true, true, now(), now()),
        ...
        print(f"[*] INSERT {SCHEMA}.productos ({len(products_data)} filas)")
        ...
        if aliases_data:
            print(f"[*] INSERT {SCHEMA}.productos_aliases ({len(aliases_data)} filas)")
        ...
        for list_id in range(1, N_LISTAS + 1):
            ...
            print(f"[*] INSERT {SCHEMA}.precios_productos lista {list_id}: {len(prices_data)}")
    ...
    try:
        ...
        if resp.status_code >= 400:
            print("[WARN] vectorize no OK; el catálogo igual quedó cargado")

...
- **Memoria conversacional (schema `core`)** — `core.conversations`, `core.conversation_events`, ventana de contexto del LLM (fuente única unificada). Sistemas: agente, backend. Evidencia: agente `app/memory/store.py`, spec 013 (unificación completada).
